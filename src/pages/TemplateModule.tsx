...
import { 
  ...
  isWordApiAvailable, 
  SUMMARY_CONFIG 
} from '../services/wordApiService';

// Word API helpers moved to WordApiService
...
export const TemplateModule: React.FC = () => {
  ...
  const insertContentControl = (id: string, label: string) => WordApiService.insertContentControl(id, label);
  const insertSummaryTable = (type: string) => WordApiService.insertSummaryTable(type);
  const insertParticipantTable = (role: string) => WordApiService.insertParticipantTable(role);
